...
| `stellar_accounts.py` | Manage Stellar CLI identities: `create` / `address` / `list` / `balance` / `fund` (friendbot on testnet; QR PNG popup + SEP-0007 URI on mainnet, with optional `--amount` / `--memo`; polls Horizon for receipt) / `drain` (return the balance to the most recent native sender, keeping a configurable reserve) / `estimate` (XLM needed to deploy a specific contract, from a live Soroban simulation) |
