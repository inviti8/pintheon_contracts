...
| `stellar_accounts.py` | Manage Stellar CLI identities: `create` / `address` / `list` / `balance` / `fund` (friendbot on testnet; QR PNG popup + SEP-0007 URI on mainnet, with optional `--amount` / `--memo`; polls Horizon for receipt) / `estimate` (XLM needed to deploy a specific contract, from a live Soroban simulation) |
